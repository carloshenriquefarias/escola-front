...
export default function EditProfileClientsAndBrokers() {
  ...
  const userIdCurrent = userInfo && userInfo.id ? userInfo.id.toString() : '';
  ...
  const fetchUserLogged = async () => {
    ...
    try {
      const response = await api.get(`/user/me.php?id=${userInfo?.id}`);
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      // formDataToSend.append("id", userInfo.id);
      formDataToSend.append("id", userInfo?.id || "");
