...
import { Aluno, DashboardData } from '../dtos/types';
import { useDisclosure } from '@chakra-ui/react'; // Adicionar useDisclosure
...
export default function AllStudents() {
  ...
  const fetchAllStudents = async (page: number) => {
    ...
    try {
      ...
      setAllStudents(response.data.alunos.data);
      setCurrentPage(response.data.alunos.current_page);
      setTotalPages(response.data.alunos.last_page);

    } catch (error) {
      console.error('Error:', error);
      setAllStudents([]);
      toastApiResponse(error, 'Não foi possível listar os alunos!');
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.error('Error:', error);
      toastApiResponse(error, 'Erro ao carregar dados do dashboard!');
  ...
  const handleStudentDetails = (studentId: number) => navigate(`/student_details/${studentId}`);
  ...
  return (
    <Flex direction="column" minHeight="100vh" bg="gray.50">
      <Header>
        <HeaderSection textColorSecondary={textColorSecondary} />
        ...
        <SearchSection
          ...
          onOpenAdvancedFilter={onOpen} // Passar função para abrir o modal
        />
        ...
        <StudentsDisplay
          ...
          selectedView={selectedView}
          onStudentClick={handleStudentDetails}
          onGeneratePDF={() => {/* Adicionar lógica de PDF aqui se necessário */ }}
        />
        ...
        <AdvancedFilterModal
          isOpen={isOpen} // Passar estado do modal
          onClose={onClose} // Passar função de fechar
          setSearchResults={setSearchResults}
          setIsSearching={setIsSearching}
          setLoading={setLoading}
        />
