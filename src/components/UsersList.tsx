import { Fragment } from 'react';
import { Container, Flex, Stack, VStack, Icon, Divider, useColorModeValue, Avatar, Text} from '@chakra-ui/react';
import { BsDot } from 'react-icons/bs';

interface Notification {
  notification: string;
  dateTime: string;
  planSelected: string;
  userName: string;
  email: string;
  userAvatar: string;
  isOnline: boolean;
}

const notifications: Notification[] = [
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'rike@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: false
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Alan Andrade',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Kent Dodds',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/kent-c-dodds',
    isOnline: true
  },
  {
    notification: 'Carlos Henrique',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Dan Abrahmov',
    email: 'alan@email.com',
    userAvatar: 'https://bit.ly/dan-abramov',
    isOnline: true
  },
  {
    notification: 'Ana Maria',
    dateTime: '10/04/2024',
    planSelected: 'Master',
    userName: 'Jena Karlis',
    email: 'alan@email.com',
    userAvatar:
      'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=334&q=80',
    isOnline: false
  }
];

const UsersList = () => {
  return (
    <Container maxW="100%" p={{ base: 2, md: 2 }}>
      <VStack
        boxShadow={useColorModeValue(
          '2px 6px 8px rgba(160, 174, 192, 0.6)',
          '2px 6px 8px rgba(9, 17, 28, 0.9)'
        )}
        bg={useColorModeValue('gray.50', "navy.700")}
        rounded="md"
        overflow="hidden"
        spacing={0}
      >
        {notifications.map((notification, index) => (
          <Fragment key={index}>
            <Flex
              w="100%"
              justifyContent="space-between"
              alignItems="center"
              _hover={{ bg: useColorModeValue('white', 'white') }}
            >
              <Stack spacing={0} direction="row" alignItems="center">
                <Flex p={4}>
                  <Avatar size="md" name={notification.userName} src={notification.userAvatar} />
                </Flex>
                <Flex direction="column" p={2}>
                  <Text
                    color={useColorModeValue('blue.300', 'white')}
                    fontSize={{ base: 'sm', sm: 'md', md: 'lg' }}
                    fontWeight={'semibold'}
                    dangerouslySetInnerHTML={{ __html: notification.notification }}
                  />
                  <Text
                    color={useColorModeValue('gray.400', 'gray.200')}
                    fontSize={{ base: 'sm', sm: 'md' }}
                  >
                    Registered at: {notification.dateTime}
                  </Text>
                  <Text
                    color={useColorModeValue('gray.400', 'gray.200')}
                    fontSize={{ base: 'sm', sm: 'md' }}
                  >
                    Plan selected: {notification.planSelected}
                  </Text>
                  <Text
                    color={useColorModeValue('gray.400', 'gray.200')}
                    fontSize={{ base: 'sm', sm: 'md' }}
                    fontWeight={'thin'}
                  >
                    {notification.email}
                  </Text>
                </Flex>
              </Stack>
              {notification.isOnline && (
                <Flex p={4}>
                  <Icon as={BsDot} w={10} h={10} color="blue.400" />
                </Flex>
              )}
            </Flex>
            {notifications.length - 1 !== index && <Divider m={0} />}
          </Fragment>
        ))}
      </VStack>
    </Container>
  );
};

export default UsersList;